...
api_key = '99299a49ab3aa2f8538c3d11f5662488',
...
function getWeatherData(lat, lon, name, country, state){
    ...
    fetch(WEATHER_API_URL).then(res => res.json()).then(data => {
        ...
        <div class="card-footer">
            <p><i class="fa-light fa-calendar"></i>${days[date.getDay()]}, ${date.getDate()}, ${months[date.getMonth()]} ${date.getFullYear()}</p>  <br>
    ...
    fetch(FORECAST_API_URL).then(res => res.json()).then(data => {
        ...
        <div class="hourly-forecast">
                <div class="card">
                    ...
                    <p>${(hourlyForecast[1].main.temp - 273.15).toFixed(2)}&deg;C</p>
                </div>
        `;
        }
...
function getCityCoordinates(){
    ...
    fetch(GEOCODING_API_URL_).then(res => res.json()).then(data => {
        ...
        getWeatherData(lat, lon, name, country, state);
}).catch(() => {
...
function getUserCoordinates(){
    navigator.geolocation.getCurrentPosition(position => {
        ...
        fetch(REVERSE_GEOCODING_API_URL).then(res => res.json()).then(data => {
            ...
            getWeatherData(latitude, longitude, name, country, state);
        }).catch(() => {
            alert('Failed to fetch user coordinates');
        });
    }, error => {
        if (error.code == error.PERMISSION_DENIED){
            alert('Geolocation permission denied. Please reset location permsission to grant access again');
        }
    });
}
